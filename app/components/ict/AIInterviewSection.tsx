          ...
          <div className="md:order-1">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="text-[11px] font-bold tracking-[0.12em] uppercase text-blue-600 bg-blue-50 border border-blue-200/50 px-3 py-0.5 rounded-full">
                {t.ict.aiInterviewSection.badge}
              </span>
            </div>
            <h2
            ...
            >
              {t.ict.aiInterviewSection.title
                .split('\n')
                .map((line, i) => (
                  <span key={i}>
                    {line}
                    {i <
                      t.ict.aiInterviewSection.title.split('\n').length -
                        1 && <br />}
                  </span>
                ))}
            </h2>
            <p className="text-[15px] text-slate-600 leading-[1.9] mb-7">
              {t.ict.aiInterviewSection.description
                .split('\n')
                .map((line, i) => (
                  <span key={i}>
                    {line}
                    {i <
                      t.ict.aiInterviewSection.description.split('\n')
                        .length -
                        1 && <br />}
                  </span>
                ))}
            ...
            <ul className="grid grid-cols-2 gap-x-4 gap-y-2.5 list-none mb-8">
              {t.ict.aiInterviewSection.features.map((f) => (
                <li
                  key={f}
