...
function OffshoreIllustration() {
  return (
    <svg viewBox="0 0 540 400" xmlns="http://www.w3.org/2000/svg" className="block w-full h-auto">
      ...
      <text x="270" y="230" fontFamily="sans-serif" fontSize="60" fontWeight="900" fill="#0ea5e9" opacity="0.07" textAnchor="middle" letterSpacing="-2">OFFSHORE</text>
      ...
      <div className="max-w-[1080px] mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              {t.ict.offshoreSection.title.split('\n').map((line, i) => (
                <span key={i}>
                  {line}
                  {i < t.ict.offshoreSection.title.split('\n').length - 1 && (
                    <br />
                  )}
                </span>
              ))}
            ...
            <p className="text-[15px] text-slate-600 leading-[1.9] mb-7">
              {t.ict.offshoreSection.description
                .split('\n')
                .map((line, i) => (
                  <span key={i}>
                    {line}
                    {i <
                      t.ict.offshoreSection.description.split('\n').length -
                        1 && <br />}
                  </span>
                ))}
            </p>
            <ul className="grid grid-cols-2 gap-x-4 gap-y-2.5 list-none mb-8">
              {t.ict.offshoreSection.features.map((f) => (
                <li
                  key={f}
                  className="flex items-start gap-2 text-[13.5px] text-slate-600 leading-snug"
                >
